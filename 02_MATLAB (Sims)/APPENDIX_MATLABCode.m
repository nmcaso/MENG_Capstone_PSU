...
config.     N           = 2125; %size(leafdata.rfdata,1);                   % number of points per DAS imaging 'frame'
config.     T           = config.N*config.dt;                               % [s] total time for 1 frame
config.     t           = 0:config.dt:config.T;                             % [s] time vector
config.     f           = 1e6;                                              % [Hz] pulsed laser frequency
...
model.      PML         = true;                                             % switch for absorbing layer boundaries (false = periodic)
            ...
            %model dimensions
model.      widthfac    = 1.2;                                                % (model width)/(size of transducer)
model.      dens        = 5;                                                % number of points per wavelength in all directions
...
source.     p1(zsubs)   = model.cfl(3)*pfun(config.t(2));                   %[Pa]
            end
...
disp("time per iteration: " + timedone/config.N )
